...
import { CheckCircle, ShieldCheck, Zap, ArrowRight, Loader2, Calendar, Globe, Mail } from 'lucide-react';
...
const AuditLanding: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. Envoyer les données à Formspree (Email officiel)
    const formData = new FormData();
    formData.append('Nom', form.name);
    formData.append('Entreprise', form.company);
    formData.append('Email', form.email);
    formData.append('WhatsApp', form.phone);
    formData.append('Objectif', form.objective);
    formData.append('_subject', 'NOUVEL AUDIT TECHNIQUE - IMANI-TECH');

    try {
      await fetch(`https://formspree.io/f/${OFFICIAL_EMAIL}`, {
        method: 'POST',
        body: formData,
        headers: {
      ...
      console.warn("L'envoi email a échoué, mais on continue l'audit IA.");
    }

    // 2. Générer le résumé IA pour l'utilisateur
    const feedback = await generateAuditSummary(
      `Nom: ${form.name}, Entreprise: ${form.company}`,
      form.objective
    );
    setResult(feedback);
    setLoading(false);
      ...
      <div className="min-h-screen bg-brand-cream flex items-center justify-center py-20 px-4 page-appear">
        <div className="max-w-2xl w-full bg-white p-10 sm:p-16 rounded-[3rem] shadow-2xl animate-in zoom-in duration-500 border border-brand-sand relative overflow-hidden">
           ...
           <div className="text-center mb-10 relative z-10">
              ...
              <h1 className="text-4xl font-black text-brand-stone uppercase tracking-tighter leading-none">Diagnostic <br /> <span className="text-brand-orange">Enregistré !</span></h1>
           </div>
           ...
           <div className="mb-8 bg-brand-beige p-6 rounded-2xl border border-brand-sand/50 flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-brand-stone">Copie envoyée à <strong>{OFFICIAL_EMAIL}</strong></p>
              </div>
           </div>
           ...
           <div className="bg-brand-orange/5 border-l-8 border-brand-orange p-8 mb-10 rounded-r-[2rem] relative z-10">
              ...
              <p className="text-brand-stone text-base sm:text-lg leading-relaxed font-bold">
                 {result}
              </p>
           </div>
           <div className="text-center relative z-10">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link to="/" className="inline-flex items-center bg-brand-stone text-white px-10 py-4 rounded-full font-black text-sm uppercase tracking-widest hover:bg-brand-orange transition-all shadow-xl shadow-brand-stone/20">
                  Retour au site <ArrowRight size={14} className="ml-2" />
                </Link>
                <a href={WHATSAPP_LINK} target="_blank" className="inline-flex items-center bg-green-500 text-white px-10 py-4 rounded-full font-black text-sm uppercase tracking-widest hover:scale-105 transition-all shadow-lg shadow-green-500/10">
                  Besoin d'aide ?
                </a>
              </div>
           </div>
    ...
    <div className="min-h-screen bg-brand-cream text-brand-stone font-inter page-appear">
      <nav className="p-8 border-b border-brand-sand bg-brand-beige sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
           ...
           <div className="flex items-center space-x-2 text-[10px] font-black uppercase text-brand-stone/40 tracking-[0.2em] hidden sm:flex">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-32 grid grid-cols-1 lg:grid-cols-2 gap-20 lg:gap-32 items-center">
        <div className="text-center lg:text-left animate-in fade-in duration-700">
          <div className="inline-block px-4 py-1.5 bg-brand-orange/10 border border-brand-orange/20 text-brand-orange text-[10px] font-black rounded-full mb-10 uppercase tracking-[0.2em] shadow-sm">Expertise Nationale Certifiée</div>
        ...
        <div className="bg-white p-10 sm:p-16 rounded-[3rem] shadow-2xl border border-brand-sand relative overflow-hidden animate-in zoom-in duration-700">
           <div className="absolute bottom-0 right-0 w-64 h-64 bg-brand-orange/5 blur-[100px] rounded-full"></div>
           ...
           <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
              <div className="space-y-3">
                ...
                <input required type="text" value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} placeholder="Ex: Marc Atangana" className="w-full bg-brand-cream border border-brand-sand p-5 rounded-xl focus:border-brand-orange focus:bg-white focus:outline-none text-sm font-bold transition-all placeholder:text-brand-stone/30" />
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-stone/40 ml-1">Structure & Localisation</label>
                <input required type="text" value={form.company} onChange={(e) => setForm({...form, company: e.target.value})} placeholder="Ex: Sarl - Yaoundé" className="w-full bg-brand-cream border border-brand-sand p-5 rounded-xl focus:border-brand-orange focus:bg-white focus:outline-none text-sm font-bold transition-all placeholder:text-brand-stone/30" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-stone/40 ml-1">Email Pro</label>
                  <input required type="email" value={form.email} onChange={(e) => setForm({...form, email: e.target.value})} placeholder="contact@domain.cm" className="w-full bg-brand-cream border border-brand-sand p-5 rounded-xl focus:border-brand-orange focus:bg-white focus:outline-none text-sm font-bold transition-all placeholder:text-brand-stone/30" />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-stone/40 ml-1">WhatsApp</label>
                  <input required type="tel" value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} placeholder="+237 6..." className="w-full bg-brand-cream border border-brand-sand p-5 rounded-xl focus:border-brand-orange focus:bg-white focus:outline-none text-sm font-bold transition-all placeholder:text-brand-stone/30" />
                </div>
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-stone/40 ml-1">Quel est votre défi majeur ?</label>
                <textarea required value={form.objective} onChange={(e) => setForm({...form, objective: e.target.value})} rows={3} placeholder="Expliquez-nous brièvement vos attentes..." className="w-full bg-brand-cream border border-brand-sand p-5 rounded-xl focus:border-brand-orange focus:bg-white focus:outline-none text-sm font-bold resize-none transition-all placeholder:text-brand-stone/30"></textarea>
              </div>
              <button disabled={loading} type="submit" className="w-full bg-brand-orange hover:bg-brand-stone text-white py-6 rounded-xl font-black text-xl transition-all shadow-xl shadow-brand-orange/30 flex items-center justify-center disabled:opacity-50 mt-4 uppercase tracking-tighter">
                {loading ? <><Loader2 className="animate-spin mr-3" /> Analyse Stratégique...</> : "Réclamer mon Audit National"}
              </button>
              <div className="flex items-center justify-center space-x-2 text-[9px] font-black uppercase tracking-[0.2em] text-brand-stone/30">
                ...
                <span>Envoi sécurisé vers imanitechsolutions237@gmail.com</span>
              </div>
           </form>
        </div>
      </div>
